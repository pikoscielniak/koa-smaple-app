...
module.exports.registerStrategy = new LocalStrategy(strategyOptions, function (email, password, done) {
    co(function * () {
        ...
        var user = yield users.findOne(searchUser);
        if (user) throw new Error('Email alrady exists');
        var hashedPasswrd = passwordService.hash(password);
        var savedUser = yield users.insert({email: email, password: hashedPasswrd});
        return savedUser;
    }).then(done, done);
});